...
import { StyledInput } from "./addProduct/styledInput";
import { StyledDropdown } from "./addProduct/styledDropdown";
import { addProduct } from "./addProduct/network";
...
const EditDialog = (props: SimpleDialogProps) => {
  ...
  const producFetchData = async () => {
    try {
      const response = await fetch(
        "http://localhost:8080/api/product/getById?id=" + selectedItemId,
        {
          mode: "cors",
          method: "GET",
          headers: {
  ...
  const mockData = ["Shumagh", "Tashkent", "Dushanbe"];
  return (
    <>
      <Dialog onClose={handleClose} open={open} fullWidth maxWidth={"xl"}>
        <Typography>{selectedItemId}</Typography>
        <Stack
          sx={{
            display: "flex",
